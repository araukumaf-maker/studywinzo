...
if ($action === 'batches' || $action === 'all') {
    ...
    foreach ($batches as &$b) {
        $b['image_url'] = !empty($b['image']) ? $base.'/admin/uploads/batches/'.$b['image'] : '';
    }
    unset($b);
    $settings['logo_url'] = !empty($settings['logo']) ? $base.'/admin/uploads/logos/'.$settings['logo'] : '';
...
if ($action === 'popup') {
    ...
    if (!empty($popup['image'])) $popup['image_url'] = $base.'/admin/uploads/popups/'.$popup['image'];
